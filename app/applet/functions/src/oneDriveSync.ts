...
export const syncVehiclePhotoToOneDrive = functions
  ...
  .onObjectFinalized(async (object) => {
    ...
    const userPrincipalName = "jenamorado@tentelcom.com";
    const graphEndpoint = `https://graph.microsoft.com/v1.0/users/${encodeURIComponent(userPrincipalName)}/drive/root:/UNIDADES TENTELCOM/${encodeURIComponent(nombreUnidad)}/${encodeURIComponent(formattedFileName)}:/content`;
