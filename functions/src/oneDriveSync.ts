...
export const syncVehiclePhotoToOneDrive = onObjectFinalized(
  ...
  async (event) => {
    ...
    const graphEndpoint = `https://graph.microsoft.com/v1.0/drives/${SHARED_DRIVE_ID}/items/${SHARED_FOLDER_ITEM_ID}:/${encodeURIComponent(
      nombreUnidad
    )}/${encodeURIComponent(anioMes)}/${encodeURIComponent(fileName)}:/content`;
