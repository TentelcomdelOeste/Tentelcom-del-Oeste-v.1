...
interface EfficiencyPopoverProps {
  kmPorGalon: number;
  totalKm: number;
  totalGalones: number;
...
/**
 * Popover que muestra el detalle del rendimiento promedio.
 * Utiliza AnalysisPopover para un posicionamiento consistente.
 */
export const EfficiencyPopover: React.FC<EfficiencyPopoverProps> = ({ 
  kmPorGalon, 
  totalKm, 
  totalGalones,
        ...
        <div className="mb-3 sm:mb-4">
          ...
          <div className="flex items-baseline gap-1.5">
            <span className="text-2xl sm:text-3xl font-black text-blue-950 tracking-tight">
              {kmPorGalon.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </span>
            <span className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase">km/gal</span>
          </div>
        </div>
        ...
        <div className="space-y-3 sm:space-y-4 pt-3 sm:pt-4 border-t border-slate-100">
          <div>
            ...
            <p className="text-[9px] sm:text-[10px] font-medium text-slate-600 leading-relaxed italic">
              Kilómetros recorridos ÷ Galones consumidos
            </p>
          </div>
          ...
          <div className="bg-slate-50 p-2.5 sm:p-3 rounded-lg border border-slate-100">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center text-[10px] gap-0.5 sm:gap-2">
                ...
                <span className="font-bold text-slate-700">{totalGalones.toLocaleString(undefined, { maximumFractionDigits: 2 })} gal</span>
              </div>
              <div className="pt-1.5 border-t border-slate-200 mt-1 sm:mt-1.5 flex justify-between items-center">
                ...
                <span className="text-[10px] sm:text-[11px] font-black text-blue-900">{kmPorGalon.toLocaleString(undefined, { maximumFractionDigits: 2 })} km/gal</span>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <p className="text-[9px] sm:text-[10px] text-slate-500 leading-relaxed">
              Un valor más alto representa una mejor eficiencia de combustible para este vehículo. Este cálculo se basa en los registros históricos de bitácora.
            </p>
          </div>
        </div>
      </div>
